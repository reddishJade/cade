...
from xcode.cli.exit_keys import EXIT_CONFIRM_WINDOW_SECONDS, ExitKeyKind
...
def _build_tui(tmp_path: Path) -> _XcodeTui:
    root = tmp_path.resolve()
    store = SessionStore(root / ".xcode" / "sessions")
    return _XcodeTui(
        _FakeApp(store),
        root,
        input=DummyInput(),
...
def _exit_patches(tui: _XcodeTui):
    return (
        patch("xcode.cli.tui.app.print_saved_conversation"),
