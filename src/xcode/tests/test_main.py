...
from xcode.main import _run, main, parse_args
...
def _patch_main_startup(
    ...
    *,
    ...
    """隔离 main() 启动路径的外部依赖。"""
    monkeypatch.setattr("xcode.main.parse_args", lambda argv=None: parse_args([]))
    monkeypatch.setattr(
        "xcode.cli.ptk_patch.suppress_windows_ptk_shutdown_noise", lambda: None
    )
    monkeypatch.setattr("xcode.main.has_valid_config", lambda root: False)
    monkeypatch.setattr("xcode.main.sys.stdin", SimpleNamespace(isatty=lambda: isatty))
    monkeypatch.setattr("xcode.main.prompt_login_method", lambda: login_method)
    monkeypatch.setattr(
        "xcode.main.discover_runtime_config",
        lambda *args, **kwargs: SimpleNamespace(
            paths=SimpleNamespace(sessions_dir=None)
        ),
    )
    monkeypatch.setattr("xcode.main._run", lambda *args, **kwargs: 0)
...
def test_main_requires_credentials_in_non_tty(monkeypatch, capsys) -> None:
    ...
    assert main() == 1
    assert "xcode login" in capsys.readouterr().err
...
def test_main_starts_oauth_login_when_choosing_auth(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "xcode.cli.auth_cmd.handle_login_command",
...
def test_main_aborts_when_oauth_login_fails(monkeypatch) -> None:
    _patch_main_startup(monkeypatch, isatty=True, login_method="auth")
    monkeypatch.setattr("xcode.cli.auth_cmd.handle_login_command", lambda **kwargs: 1)
