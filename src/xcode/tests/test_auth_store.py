...
from xcode.cli.auth_cmd import (
...
from xcode.harness.auth.types import AuthCredential
...
def test_auth_store_lifecycle(tmp_path: Path) -> None:
    store_file = tmp_path / ".xcode" / "auth.json"
...
def test_auth_manager_auto_refresh(tmp_path: Path) -> None:
    ...
    with patch(
        "xcode.harness.auth.manager.refresh_openai_codex_token",
...
def test_cli_auth_handlers(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(
        "xcode.harness.auth.store.DEFAULT_AUTH_FILE", tmp_path / "auth.json"
    )
