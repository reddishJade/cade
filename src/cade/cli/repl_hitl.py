...
from cade.agent.types import ApprovalRequest, ApprovalScope, ToolInput, ToolSpec
...
def _ask_hitl_choice_with_timeout(
    ...
    thread = threading.Thread(target=run_prompt, name="cade-hitl-prompt", daemon=True)
    thread.start()
...
def _ask_suggestion_with_timeout(timeout: float) -> str:
    ...
    thread = threading.Thread(
        target=run_prompt, name="cade-hitl-suggestion", daemon=True
    )
    thread.start()
