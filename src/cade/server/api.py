"""Cade Web 服务：REST + WebSocket API 与静态前端。"""
...
def create_app(
    ...
    server = FastAPI(title="cade web", docs_url=None, redoc_url=None)
    hub = WebRunHub(app)

    server.state.cade_app = app
...
def _info_payload(app: CadeApp, project_root: Path) -> dict[str, object]:
    payload: dict[str, object] = {
...
_WORKSPACES_FILE = Path.home() / ".cade" / "web_workspaces.json"
