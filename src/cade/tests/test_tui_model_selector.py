...
from cade.cli.repl_settings import AvailableModelEntry
...
def _build_tui(tmp_path: Path) -> tuple[_CadeTui, _ModelApp]:
    root = tmp_path.resolve()
    app = _ModelApp(SessionStore(root / ".cade" / "sessions"))
    tui = _CadeTui(app, root, input=DummyInput(), output=DummyOutput())
    return tui, app
...
def test_tui_model_command_uses_native_escape_menu(tmp_path: Path) -> None:
    ...
    with patch(
        "cade.cli.repl_settings.get_available_model_entries",
...
def test_tui_model_command_switches_selected_model(tmp_path: Path) -> None:
    ...
    with patch(
        "cade.cli.repl_settings.get_available_model_entries", return_value=[entry]
    ):
        tui._show_native_command_choice("/model")
