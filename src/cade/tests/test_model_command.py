...
from cade.cli.repl_settings import (
...
def test_cade_app_set_model_smart_inference() -> None:
    from unittest.mock import MagicMock

    from cade.ai.providers.registry import ModelProfileConfig
    from cade.ai.resolver import ModelResolver
...
def test_cade_app_rejects_unsupported_model_effort() -> None:
    from unittest.mock import MagicMock
...
def test_terminal_isolated_guard() -> None:
    from cade.cli.ptk_patch import terminal_isolated

    with (
        patch("cade.cli.ptk_patch.get_console_mode", return_value=0x1234),
        patch("cade.cli.ptk_patch.set_console_mode") as mock_set,
        patch("cade.cli.ptk_patch.flush_console_input_buffer") as mock_flush,
    ):
...
def test_prompt_session_adapter_flushes_buffer() -> None:
    ...
    from cade.cli.repl_rendering import PromptSessionAdapter

    mock_raw_session = MagicMock()
    mock_raw_session.prompt.return_value = "hello"

    adapter = PromptSessionAdapter(mock_raw_session)
    with patch("cade.cli.ptk_patch.flush_console_input_buffer") as mock_flush:
        val = adapter.prompt("> ")
        assert val == "hello"
        assert mock_flush.called
