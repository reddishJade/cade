...
from cade.coding_agent.assembly.security import sandbox_policy_from_security
...
def test_existing_and_missing_cade_metadata_are_protected(tmp_path: Path) -> None:
    project = tmp_path / "project"
    project.mkdir()
    metadata = project / ".cade"
    metadata.mkdir()
    ...
    metadata.rename(project / ".cade-existing")
    missing = shell.run(
        ["sh", "-c", "mkdir .cade && printf '{}' > .cade/settings.json"],
        project,
    )
