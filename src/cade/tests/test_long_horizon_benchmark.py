...
from cade.harness.agent_runtime.events import FinalStructuredEvent
...
def test_report_uses_same_complete_pair_cohort_for_values_and_change() -> None:
    records = [
        ...
        _record("cade", 500, True, True, repeat=1),
        _record("baseline", 2000, True, True, repeat=2),
        {
            **_record("cade", 400, True, False, repeat=2),
    ...
    assert summary["variants"]["cade"]["input_tokens_mean"] == 500
...
def test_report_selects_first_complete_pair_attempt() -> None:
    records = [
        ...
        {
            **_record("cade", 200, True, False, attempt=1),
            "usage_incomplete_calls": [
                {
                    "call_index": 1,
                    "kind": "agent",
        ...
        _record("cade", 450, True, True, attempt=2),
    ]
    ...
    assert summary["variants"]["cade"]["input_tokens_mean"] == 450
...
def test_report_keeps_post_rollover_pair_when_total_usage_is_incomplete() -> None:
    ...
    cade = {
        **_record("cade", 0, True, False),
        "post_rollover_input_tokens": 300,
        "post_rollover_usage_complete": True,
    }

    summary = summarize_records([baseline, cade])
...
def test_cli_retries_the_whole_pair_and_preserves_attempts(
    ...
    def fake_run_task(
        ...
        incomplete = variant == "cade" and attempt == 1
        return {
    ...
    records = benchmark_cli._run_variants(args, ("baseline", "cade"))
...
def test_report_rejects_mismatched_pair_controls() -> None:
    ...
    cade = {**_record("cade", 500, True, True), "model": "model-b"}

    with pytest.raises(ValueError, match="differ on control model"):
        summarize_records([baseline, cade])
