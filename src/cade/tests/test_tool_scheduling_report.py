...
def test_report_computes_per_task_p50_p95_and_paired_speedup(
    ...
    assert stats["cade_p50_seconds"] == 0.275
    ...
    assert quality["cade_run_success_rate"] == 1.0
...
def test_report_excludes_failed_or_output_mismatched_pairs(tmp_path: Path) -> None:
    records = [
        _record("serial", 1, 1.0),
        _record("cade", 1, 0.25, digest="different"),
        _record("serial", 2, 1.1),
        _record("cade", 2, 0.3, success=False),
    ]
    ...
    quality = summary["quality"]
    assert isinstance(quality, dict)
    assert quality["cade_run_success_rate"] == 0.5
