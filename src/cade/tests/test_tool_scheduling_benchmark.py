...
async def test_measure_scheduling_uses_production_parallel_execution(
    ...
    cade = await measure_scheduling(task, "cade", repeat=1, workspace=tmp_path / "cade")

    assert serial.success
    assert cade.success
    assert serial.max_concurrency == 1
    assert cade.max_concurrency == 4
    assert cade.duration_seconds < serial.duration_seconds * 0.7
    assert cade.output_digest == serial.output_digest
    assert cade.workspace_digest == serial.workspace_digest
...
async def test_mixed_workload_preserves_write_barriers(tmp_path: Path) -> None:
    ...
    result = await measure_scheduling(
        task, "cade", repeat=1, workspace=tmp_path / "run"
    )
